...
def _prepare_stream(stream, event, extract_len, pre_pick, seed_pick_ids=None):
    ...
    for seed_pick_id in seed_pick_ids:
        ...
        #print(stream.select(station = seed_pick_id.seed_id.split('.')[1]))
        #print(seed_pick_id.seed_id.split('.')[3])
        #Edit here as the names of the seed do not match the names of my streams
        # Original logic (commented for better channel handling):
        # if seed_pick_id.seed_id.split('.')[3] == 'N':
        #     try:
        #         tr = stream.select(station = seed_pick_id.seed_id.split('.')[1],
        #                            component = '1').merge()
        #         id2 = tr[0].get_id()
        #     except:
        #         try:
        ...
        # Modified: Better channel selection logic based on expected channel code
        expected_channel = seed_pick_id.seed_id.split('.')[3]  # Get expected channel (EHE, HHE, EHZ, etc.)
        station_code = seed_pick_id.seed_id.split('.')[1]
        
        # Extract the orientation (last character: Z, E, N, 1, 2)
        orientation = expected_channel[-1]
        
        try:
            # First try exact channel match
            tr = stream.select(station=station_code, channel=expected_channel).merge()
            if len(tr) > 0:
                id2 = tr[0].get_id()
            else:
            ...
            try:
                # Fallback: try by orientation only
                tr = stream.select(station=station_code, component=orientation).merge()
                if len(tr) > 0:
                    id2 = tr[0].get_id()
                else:
                    raise Exception("No orientation match")
            except:
                try:
                    # Additional fallbacks for specific cases
                    if orientation in ['E', 'N']:
                        # For horizontal channels, try alternative orientations
                        for alt_orient in ['1', '2', 'E', 'N']:
                            tr = stream.select(station=station_code, component=alt_orient).merge()
                            if len(tr) > 0:
                                id2 = tr[0].get_id()
                                break
                        else:
                            raise Exception("No horizontal match")
                    elif orientation == 'Z':
                        # For vertical channels, try U as alternative
                        tr = stream.select(station=station_code, component='U').merge()
                        if len(tr) > 0:
                            id2 = tr[0].get_id()
                        else:
                            raise Exception("No vertical match")
                    else:
                        raise Exception("No fallback available")
                except:
                    print('fail, ' + str(seed_pick_id))
                    continue        #print(tr)
        #tr = stream.select(station = seed_pick_id.seed_id.split('.')[1],component = [comp]).merge()
        #print('trace:',tr[0].get_id())
        if len(tr) == 0:
            continue
        else:
            tr = tr[0]
        Logger.debug(
            ...
            f"{tr.stats.endtime} to {pick.time - pre_pick} - "
            f"{(pick.time - pre_pick) + extract_len}")
        tr = stream.select(id=id2).slice(
            starttime=pick.time - pre_pick,
            endtime=(pick.time - pre_pick) + extract_len).merge()
        
        #print('trace_trimmed =', tr)
        #print(len(tr))
        if len(tr) == 0:
            continue
        #print('trace id before: ' ,tr[0].id)
        #print(tr)
#This next line ensure the issue above about the trace id and the seed id being different is not an issue for the rest of the code
        tr[0].id = seed_pick_id.seed_id
        #print('trace id after: ' ,tr[0].id, '.  SEED ID: ', seed_pick_id.seed_id)
        if len(tr) == 0:
            continue
        if len(tr) > 1:
            Logger.error("Multiple traces for {seed_id}".format(
            ...
            continue
        tr = tr[0]
        # Original strict length check (commented for DPRI station compatibility):
        # if tr.stats.endtime - tr.stats.starttime != extract_len:
        #     Logger.warning(f"Insufficient data for {tr.id}, discarding")
        #     continue
        # Modified: Check if data length is sufficient with tolerance for sampling differences
        actual_len = tr.stats.endtime - tr.stats.starttime
        # Increased tolerance for problematic stations (was 0.1, now 0.5 for 50% minimum)
        tolerance = 0.5  # Accept streams with at least 50% of expected length (0.5s for 1s window)
        if actual_len < extract_len - tolerance:
            Logger.warning(f"Insufficient data for {tr.id}: {actual_len:.3f}s < {extract_len-tolerance:.3f}s, discarding")
            continue
        stream_sliced.update(
            {seed_pick_id.phase_hint:
             stream_sliced[seed_pick_id.phase_hint] + tr})
...
def _compute_dt_correlations(catalog, master, min_link, event_id_mapper,
    ...
    #print(stream_dict[str(master.resource_id)])
    ...
    for phase_hint in master_stream.keys():  # Loop over P and S separately
        for sampling_rate in sampling_rates:  # Loop over separate samp rates
            ...
            # Original strict subset check (commented for more flexible correlation):
            ...
            # Modified: Check for at least some overlap instead of strict subset
            common_seed_ids = master_seed_ids.intersection(matched_seed_ids)
            if len(common_seed_ids) == 0:
                Logger.warning(
                    "No common traces found for correlation: "
                    f"master: {master_seed_ids}, matched: {matched_seed_ids}")
                continue
            elif len(common_seed_ids) < len(master_seed_ids):
                Logger.debug(
                    f"Partial trace overlap: {len(common_seed_ids)}/{len(master_seed_ids)} common traces. "
                    f"Common: {common_seed_ids}")
            
            # Filter streams to only include common traces
            # Fix: ObsPy select() doesn't accept list of IDs, filter manually
            _master_stream_filtered = Stream()
            for tr in _master_stream:
                if tr.id in common_seed_ids:
                    _master_stream_filtered.append(tr)
            _master_stream = _master_stream_filtered
            
            # Filter matched streams and update event IDs list accordingly
            used_matched_streams_filtered = []
            used_event_ids_filtered = []
            for i, st in enumerate(used_matched_streams):
                st_filtered = Stream()
                for tr in st:
                    if tr.id in common_seed_ids:
                        st_filtered.append(tr)
                if len(st_filtered) > 0:
                    used_matched_streams_filtered.append(st_filtered)
                    used_event_ids_filtered.append(used_event_ids[i])  # Keep corresponding event ID
            used_matched_streams = used_matched_streams_filtered
            used_event_ids = used_event_ids_filtered
            # Do the correlations
            Logger.debug(
            ...
            for i, used_event_id in enumerate(used_event_ids):
                for j, chan in enumerate(used_chans[i]):
                    ...
                    if cc_max < min_cc:
                        continue
                    shift -= shift_len
                    #print(phase_hint,chan.channel[0],chan.channel[1])
                    ...
                    #print(event_dict[used_event_id].picks)
                    #print(event_dict,pick)
    ...
    with open("backup_dt_test_comb.cc", "a") as f:
        for linked_event in differential_times:
            if linked_event is not None:
                #print(linked_event)
                #print(linked_event.cc_string)
...
def compute_differential_times(catalog, correlation, stream_dict=None,
                               ...
                               max_trace_workers=1, existing_corr_file = None, *args, **kwargs):
    """
    ...
    #print('DISTANCES2:',distance_filter)
    ...
    if correlation:
        ...
        if max_workers == 1:
            ...
            for i, master in enumerate(sparse_catalog):
                print(str(i) + '/' + str(len(sparse_catalog)))
                ...
                if existing_corr_file is not None and master_id in existing_pairs:
                    sub_catalog = [ev for j,ev in enumerate(sub_catalog)
                                   if ev.resource_id not in existing_pairs[master_id]]
                    Logger.info(
                ...
                try:
                    ...
                    print('Correlations failed for event ' + str(i))
                #print(differential_times)
        else:
...
def write_correlations(catalog, stream_dict, extract_len, pre_pick,
                       ...
                       parallel_process=False, existing_corr_file=None, *args, **kwargs):
    """
    ...
    correlation_times, event_id_mapper = compute_differential_times(
        ...
        interpolate=interpolate, all_horiz=all_horiz,existing_corr_file=existing_corr_file, **kwargs)
