...
xml_path = join(basedir, 'CATALOGS/MAGNITUDES', 'JAN24_SEP25_MAGNITUDES.xml')  # Ajusta el nombre
...
for event in cat:
    ...
    ot = origin.time
    year = str(ot.year)
    datetime_str = ot.strftime('%Y-%m-%d %H:%M:%S.%f')[:-2]  # truncar a 4 decimales
...
print(f'\nDone. Generated {len(events_by_year)} CSV files.')