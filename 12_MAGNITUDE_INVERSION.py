...
def summarize_amplitudes(
    ...
    i = 0
    bar = ProgressBar(maxval=total_observations)
    bar.start()
    for ev in catalog:
        ...
        for amplitude in ev.amplitudes:
            ...
            out.loc[i, "event_id"] = rid
            # out.loc[i, "origin_time"] = ori_time
            out.loc[i, "depth"] = depth
            # out.loc[i, "latitude"] = lat
            # out.loc[i, "longitude"] = lon 
            out.loc[i, "epicentral_distance"] = epi_dist
            out.loc[i, "seed_id"] = amplitude.waveform_id.get_seed_string()
            out.loc[i, "amplitude"] = amp
            out.loc[i, "period"] = amplitude.period
            out.loc[i, "seisan_magnitude"] = (
...
def find_matching_events(
    ...
    bar = ProgressBar(maxval=len(df_1))
    bar.start()
    matched_quakes = dict()
    for i in range(len(df_1)):
        ...
        # Handle None depths (skip depth check if either is None)
        if df_1.depth[i] is None or df_2.depth[index] is None:
            depth_sep = 0  # Skip depth check but continue with location check
        else:
...
def magnitude_inversion(
    ...
    # OPTIMIZED: Changed from float64 to float32 (50% memory reduction)
    Y = np.zeros(total_length, dtype=np.float32)  # WAS: dtype=np.float64
    # Construct Y
    ...
    ########################## Construct X matrix (OPTIMIZED SPARSE) ##############################
    print(f"  Memory optimization: Building sparse matrix ({total_length} x {n_new_events + n_seed_ids + 1})")
    ...
    if frequency_dependent:
        freq_dist = (1 / mag_data.period.to_numpy()) * slantdist
    else:
    ...
    seed_id_array = mag_data.seed_id.to_numpy()
    ...
    if not check1 or not check2:
        ...
        if not check1 or not check2:
            if error1 > 0.1 or error2 > 0.1:
                raise AssertionError(f"Matrix inverse error too large: error1={error1:.2e}, error2={error2:.2e}")
    ...
    if only_matched:
        ...
        bar = ProgressBar(maxval=len(new_catalog))
        bar.start()
        ...
        bar = ProgressBar(maxval=len(used_event_ids))
        bar.start()
        i = 0
    ...
    # Filter out None events (unmatched events when only_matched=False)
    callibrated_events = [ev for ev in callibrated_events if ev is not None]
...
if __name__ == "__main__":
    ...
    print(f"Code execution time: {code_end_time - code_start_time}")