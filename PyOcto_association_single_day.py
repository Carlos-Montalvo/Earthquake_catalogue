...
def pick_removal(catalog, assignments, pick_df):
    ...
    # No reset de índices para mantener la coherencia de pick_idx
...
def associate_picks_single_day(julian_day, year, picks_type, basedir, datadir, 
    ...
    # Directorios para guardar resultados por iteración
    events_iter_dir = join('/Volumes/GeoPhysics_49/users-data/montalca/CATALOGS/PYOCTO/EVENTS', str(year), 'iterations')
    assignments_iter_dir = join('/Volumes/GeoPhysics_49/users-data/montalca/CATALOGS/PYOCTO/ASSIGNMENTS', str(year), 'iterations')
    makedirs(events_iter_dir, exist_ok=True)
    makedirs(assignments_iter_dir, exist_ok=True)

    # Contador de iteraciones
    current_picks = picks.copy()
    iteration_count = 0
    global_event_idx = 0  # Contador global de eventos
    ...
    initial_associator = pyocto.OctoAssociator.from_area(
        ...
        location_split_return=5, n_threads=5,
    )
    ...
    for n_picks in range(150, 20, -2):
        ...
        if len(events) > 0:
            ...
            # Reasignar índices globales
            n_events = len(events_df_iter)
            events_df_iter['idx'] = range(global_event_idx, global_event_idx + n_events)
            if 'event_idx' in assignments_df_iter.columns:
                assignments_df_iter['event_idx'] = assignments_df_iter['event_idx'].apply(
                    lambda x: x + global_event_idx
                )
            global_event_idx += n_events
            # Guardar archivos de la iteración
            iter_name = f"{year}_{julian_day}_{iteration_count}"
            events_iter_file = join(events_iter_dir, f"{iter_name}.csv")
            assignments_iter_file = join(assignments_iter_dir, f"{iter_name}.csv")
            events_df_iter.to_csv(events_iter_file, index=False)
            if len(assignments_df_iter) > 0:
                columns_to_save = ['event_idx','pick_idx','residual','station','time','probability','phase']
                assignments_df_iter.to_csv(assignments_iter_file, index=False, columns=[col for col in columns_to_save if col in assignments_df_iter.columns])
            else:
                assignments_df_iter.to_csv(assignments_iter_file, index=False)
    ...
    # =========================================================================
    # Al finalizar, solo limpiar memoria y devolver resumen
    print(f'{colours.GREEN}Iteraciones finalizadas para el día {julian_day}{colours.ENDC}')
    print('Cleaning up memory after processing day...')
    del current_picks, stations, picks
    gc.collect()
    return {
        'success': True,
        'iterations': iteration_count,
        'events_dir': events_iter_dir,
        'assignments_dir': assignments_iter_dir
    }
