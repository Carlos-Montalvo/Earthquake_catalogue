...
# Bash script to pick amplitudes for seismic data day by day using 9_AMPLITUDE_PICKER.py
...
WAVEFORMS_DIR="$CATLOGDIR/RPNET/WAVEFORMS"
AMP_DIR="$CATLOGDIR/MAGNITUDES/AMPLITUDES"
STATIONS_DIR="$BASEDIR/STATIONS"
INVENTORY_FILE="$STATIONS_DIR/MORIA_GEONET.xml"
PARALLEL_JOBS=6  # Number of days to process in parallel

# Python environment
PYTHON_SCRIPT="/Volumes/GeoPhysics_49/users-data/montalca/PROGRAMS/PYTHON/EARTHQUAKE_CATALOGUE/amplitude_picker_single_day.py"
CONDA_ENV="kurtosis"
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}    AMPLITUDE PICKER - MULTI-DAY PROCESSING${NC}"
echo -e "${BLUE}==================================================${NC}"
echo ""
...
if [ ! -d "$NLL_DIR" ]; then
    echo -e "${RED}Error: NLL catalog directory $NLL_DIR does not exist!${NC}"
    exit 1
fi
...
# Create amplitude output directory if it doesn't exist
if [ ! -d "$AMP_DIR" ]; then
    echo -e "${YELLOW}Creating amplitude output directory: $AMP_DIR${NC}"
    mkdir -p "$AMP_DIR"
fi
...
export PYTHON_SCRIPT NLL_DIR WAVEFORMS_DIR AMP_DIR INVENTORY_FILE RED GREEN YELLOW NC
