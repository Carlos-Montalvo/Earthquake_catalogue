...
# KURTOSIS Processing Script - Simple parallel execution of kurtosis_single_day.py
...
PARALLEL_JOBS=6  # Number of days to process in parallel (conservative)

# Python environment
PYTHON_SCRIPT="/Volumes/GeoPhysics_49/users-data/montalca/PROGRAMS/PYTHON/EARTHQUAKE_CATALOGUE/kurtosis_single_day.py"
PYTHON_EXECUTABLE="/home/montalca/miniconda3/envs/kurtosis/bin/python"
CONDA_ENV="kurtosis"
...
display_config() {
    echo "=================================================="
    echo -e "${PURPLE}KURTOSIS PROCESSING CONFIGURATION${NC}"
    echo "=================================================="
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}    KURTOSIS PROCESSING - PARALLEL DAY PROCESSING${NC}"
echo -e "${BLUE}==================================================${NC}"
echo ""
...
if [ ! -f "$PYTHON_EXECUTABLE" ]; then
    echo -e "${RED}Error: Python executable $PYTHON_EXECUTABLE not found!${NC}"
    exit 1
fi
...
process_day() {
    ...
    local jday="$2"
    local date_str="$3"
    local LOG_FILE="$LOG_DIR/kurtosis_day_${jday}_${year}.log"
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        echo "SUCCESS:$jday:$DAY_DURATION:$date_str" >> /tmp/kurtosis_results_$$.tmp
        
        # Extract pick counts from log file if available
        local retained_picks=$(grep -o "Total retained picks: [0-9]*" "$LOG_FILE" | tail -1 | grep -o "[0-9]*" || echo "0")
        local kurtosis_picks=$(grep -o "Total kurtosis picks: [0-9]*" "$LOG_FILE" | tail -1 | grep -o "[0-9]*" || echo "0")
        echo "PICKS:$jday:$retained_picks:$kurtosis_picks:$date_str" >> /tmp/kurtosis_results_$$.tmp
    else
        echo -e "${RED}✗ [PID $$] Day $jday ($date_str) failed after ${DAY_MINUTES}m ${DAY_SECONDS}s (exit code: $exit_code)${NC}"
        echo "FAILED:$jday:$DAY_DURATION:$date_str" >> /tmp/kurtosis_results_$$.tmp
...
rm -f /tmp/kurtosis_results_*.tmp
...
for tmp_file in /tmp/kurtosis_results_*.tmp; do
    if [ -f "$tmp_file" ]; then
        while IFS=':' read -r status jday duration picks_or_extra date_str extra; do
            case $status in
                "SUCCESS")
...
echo ""
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}    KURTOSIS PROCESSING COMPLETED${NC}"
echo -e "${BLUE}==================================================${NC}"
...
echo -e "  ${YELLOW}/Volumes/GeoPhysics_49/users-data/montalca/CATALOGS/EQT_PICKS/KURTOSIS/${NC}"
echo ""
